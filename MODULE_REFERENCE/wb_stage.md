...
| `memwb_is_halt_i` | input | 1 | Halt metadata bit from MEM stage |
...
| `halted_o` | output | 1 | Asserted when a valid, non-faulting halt retires |

## Notes

Writes are suppressed when any fault/illegal flag is set or destination register is zero.
`halted_o` is generated from retire-valid + halt metadata with the same fault kill conditions.
